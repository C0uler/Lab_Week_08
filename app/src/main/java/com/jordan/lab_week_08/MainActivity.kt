package com.jordan.lab_week_08
...
import com.jordan.lab_week_08.Worker.SecondWorker
import com.jordan.lab_week_08.Worker.ThirdWorker
import com.jordan.lab_week_08.Worker.FirstWorker
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        workManager.getWorkInfoByIdLiveData(firstRequest.id)
            .observe(this) { info ->
                // Add this check
                if (info != null) {
                    // Now it's safe to use info.state
                    if (info.state == WorkInfo.State.SUCCEEDED) {
                        showResult("First process succeeded")
                    } else if (info.state == WorkInfo.State.FAILED) {
                        showResult("First process failed")
                    }
                }
            }
        ...
        workManager.getWorkInfoByIdLiveData(secondRequest.id)
            .observe(this) { info ->
                ...
                if (info != null) {
                    if (info.state == WorkInfo.State.SUCCEEDED) {
                        showResult("Second process succeeded")
